...
To study the interaction between industrial policy and agglomeration, we extend the
model to three geographic clusters: the United States, Europe, and Asia.
Product market competition remains \textbf{global} --- all firms compete in the same
Cournot game regardless of location --- but agglomeration spillovers are
\textbf{local}:
\begin{equation}
  c_{n,t}^{(g)} = c_{n,0} - \gamma \cdot N^{\mathrm{inn}}_{g,t}, \qquad
  N^{\mathrm{inn}}_{g,t} = n_{b,g,t} + n_{n,g,t},
  \label{eq:local_agglom}
\end{equation}
where $g \in \{\text{US}, \text{EU}, \text{Asia}\}$ is a firm's region and
$N^{\mathrm{inn}}_{g,t}$ counts only same-region innovative firms.
...
\textbf{agglomeration externality} that creates a wedge between the social and
private return to innovation.

\subsection{Counterfactual 1: Asian Industrial Policy}

Asia subsidizes innovation to correct the regional agglomeration externality.
The first-best Pigouvian subsidy per innovating firm equals the marginal social
benefit it confers on regional peers:
\begin{equation}
  \tau^*_{\mathrm{Asia}} = \gamma \cdot \frac{\partial N^{\mathrm{inn}}_{\mathrm{Asia}}}
                                              {\partial \mathbf{1}[\text{innovate}]}
  = \gamma \cdot N^{\mathrm{inn}}_{\mathrm{Asia}},
\end{equation}
i.e., each additional Asian innovator reduces the new-tech cost for all existing
regional innovators by $\gamma$, so the subsidy is increasing in the cluster size.

The key qualitative prediction is an \textbf{agglomeration spiral}: the subsidy
raises Asian innovation rates, which enlarges $N^{\mathrm{inn}}_{\mathrm{Asia}}$,
which further lowers costs and attracts more innovation, tilting the long-run global
industry composition toward Asia. Because competition is global, this comes partly
at the expense of US and European firms whose market shares decline.

\subsection{Counterfactual 2: EU--US Knowledge Integration}

The EU and US agree to pool their innovation spillovers --- perhaps through a
bilateral R\&D treaty or joint research infrastructure. Formally, the effective
cluster size for both blocs becomes:
\begin{equation}
  \tilde{N}^{\mathrm{inn}}_{\mathrm{EU+US},t}
  = n_{b,\mathrm{EU},t} + n_{n,\mathrm{EU},t}
  + n_{b,\mathrm{US},t} + n_{n,\mathrm{US},t}.
\end{equation}
Because the integrated cluster is larger, the new-tech cost for EU and US firms
falls by more than under the autarkic baseline, raising innovation rates in both
regions. Asia, whose spillovers remain local, sees its relative cost advantage
eroded, reshaping global industry geography in the opposite direction from
Counterfactual 1.

The two counterfactuals together illustrate the \textbf{strategic complementarity}
between agglomeration policy and geography: the equilibrium industry location depends
on which region acts first to internalize the externality or to deepen cross-border
knowledge flows.
